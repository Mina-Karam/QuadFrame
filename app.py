...
def hologram(infile, outfile, screen_below_pyramid=False):
    ...
    if capture.isOpened():
        width = capture.get(cv2.CAP_PROP_FRAME_WIDTH)   # Video width
        height = capture.get(cv2.CAP_PROP_FRAME_HEIGHT)  # Video height
        fps = capture.get(cv2.CAP_PROP_FPS)  # Frames per second

    # Retrieve parameters from form
    length = request.form['length']
    d = request.form['d']
    padding = request.form.get('padding', 0)
    ...
    if length % 2:
        length += 1  # Ensure length is even for convenience
    ...
    while True:
        ...
        # Place frames on the background
        bgd[length // 2 + d + padding:length // 2 + d + new_hgt + padding, 
            length // 2 - new_wid // 2:length // 2 + new_wid // 2] = resized_frame
        
        bgd[length // 2 - d - padding - new_hgt:length // 2 - d - padding, 
            length // 2 - new_wid // 2:length // 2 + new_wid // 2] = cv2.flip(resized_frame, -1)
        
        bgd[length // 2 - new_wid // 2:length // 2 + new_wid // 2, 
            length // 2 + d + padding:length // 2 + d + new_hgt + padding] = cv2.flip(cv2.transpose(resized_frame), 0)
        
        bgd[length // 2 - new_wid // 2:length // 2 + new_wid // 2, 
            length // 2 - d - padding - new_hgt:length // 2 - d - padding] = cv2.flip(cv2.transpose(resized_frame), 1)
...
def upload_file():
    """Handle file upload and hologram conversion"""
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            sp.call([
                'ffmpeg', '-i', outpath, '-i', filepath, '-c:v', 'copy', '-c:a', 'aac', 
                '-map', '0:v:0', '-map', '1:a:0?', os.path.join(uploadpath, 'out.avi')
            ])
